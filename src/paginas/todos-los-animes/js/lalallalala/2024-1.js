...
const cardsData = [
  {
    ...
    altImageSrc: "../img/2024/kimetsu-no-yaiba-hashira-geiko-hen.jpg",
    dateAdded: "2024",
    ...
    <div class="p-4 sm:w-1/2 lg:w-1/4 ${visto ? 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-white' : por_ver ? 'bg-gradient-to-b from-red-500 to-red-600 text-white' : mirando ? 'bg-gradient-to-b from-green-500 to-green-600 text-white' : 'bg-white text-black'} mx-auto">
      <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
        <a href="${cardData.readMoreLink}">
          <img class="lg:h-72 md:h-48 w-full object-cover object-center" src="${cardData.imageSrc || cardData.altImageSrc}" alt="blog">
        </a>
        <div class="p-6">
...
function addFilteredCardsToContainer(cardsData) {
  const cardsContainer = document.getElementById("cardsContainer");
  cardsContainer.innerHTML = ""; // Limpiar el contenedor antes de agregar nuevas tarjetas
  cardsData.forEach(card => addCard(card));
}

// Función para filtrar tarjetas por género y agregarlas al contenedor
function filterByGenre(genre) {
  const filteredCards = cardsData.filter(card => card.genres.includes(genre));
  addFilteredCardsToContainer(filteredCards);
}

// Función para mostrar todas las tarjetas
function showAllCards() {
  addFilteredCardsToContainer(cardsData);
}

// Identificar la página actual
var pagina = "todas"; // Esto debe ser definido en cada HTML según la página actual

// Filtrar y agregar tarjetas según la página actual
if (pagina === "accion") {
  filterByGenre("Acción");
} else {
  showAllCards();
}
